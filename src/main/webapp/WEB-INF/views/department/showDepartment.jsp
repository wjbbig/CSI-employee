            ...
            <div class="layui-form-item">
                <div class="layui-inline">
                    ...
                    <div class="layui-input-block">
                        <input type="text" id="deptNameText" placeholder="请输入" autocomplete="off" class="layui-input">
                    </div>
                </div>
                <div class="layui-inline">
...
<script>
    layui.use(['layer','form','table'], function(){
        ...
        function to_page(pageNo) {
            $.ajax({
                url:"${APP_PATH}/dept/getDepartments.do",
                data:{
                    "pageNo":pageNo,
                    "deptName":$("#deptNameText").val()
                },
                type:"post",
        ...
        function build_users_table(result) {
            ...
            $.each(users,function (index,item) {
                ...
                var deptNameTd = $("<td></td>").append(item.name);
                var deptDescTd = $("<td></td>").append(item.deptDesc);
        ...
        $(document).on("click",".delBtn",function () {
            ...
            layer.confirm("确定要删除["+name+"]的信息吗？",function (cindex) {
                layer.close(cindex);
                $.ajax({
                    url:"${APP_PATH}/dept/deleteDept.do",
                    type:"post",
                    data:{
                        "ids":id
                    },
        ...
        $("#delSelectBtn").click(function () {
            if($(".select-item:checked").length == 0){
                layer.msg("请至少选择一个部门！",{time:2000,icon:0});
                return false;
            }
            var deptNames = "";
            var deptIds = "";
            $.each($(".select-item:checked"),function () {
                deptNames += $(this).parents("tr").find("td:eq(1)").text()+",";
                deptIds += $(this).attr("id")+"-";
            });
            deptNames = deptNames.substring(0,deptNames.length-1);
            deptIds = deptIds.substring(0,deptIds.length-1);
            layer.confirm("确定要删除["+deptNames+"]的信息吗？",function (cindex) {
                layer.close(cindex);
                $.ajax({
                    url:"${APP_PATH}/dept/deleteDept.do",
                    type:"post",
        ...
        $(document).on("click",".editBtn",function () {
            ...
            layer.open({
                type: 2
                ,title: '更新部门信息'
                ,content: "${APP_PATH}/dept/toUpdateDept.go?id="+id
                ,area: ['420px', '420px']
                ,btn: ['更新', '取消']
                ,yes: function(index, layero){
                    ...
                    var deptDesc = body.find('#deptDesc').val();
                    var deptId = body.find('#deptId').val();
                    $.ajax({
                        url:"${APP_PATH}/dept/updateDept.do",
                        type:"post",
                        data:{
                            "id":deptId,
                            "name":deptName,
